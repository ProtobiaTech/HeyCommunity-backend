                        ...
                        <div class="list-group">
                            <div class="list-group-item active">
                                ...
                                <div class="pull-right section-editbox {{ Input::get('edit') === 'true' ? 'disnone' : 'disnone' }}">
                                    ...
                                    {!! Form::open(['url' => url('/admin/topic/node-destroy'), 'id' => ('form-rootNode-' . $rootNode->id), 'class' => 'hidden', 'method' => 'post']) !!}
                                        <input type="hidden" name="id" value="{{ $rootNode->id }}">
                                        {!! Form::submit('Submit', ['class' => 'btn btn-xs']) !!}
                                    {!! Form::close() !!}

                                    &nbsp;&nbsp;
                                    <a href="{{ url('admin/topic/node-move-left?id=' . $rootNode->id) }}" {{ $k === 0 ? 'disabled' : '' }} class="btn btn-default btn-xs">
                                        <i class="glyphicon glyphicon-arrow-left"></i>
                                    </a>
                                    <a href="{{ url('admin/topic/node-move-right?id=' . $rootNode->id) }}" {{ $k === ($rootNodes->count() - 1) ? 'disabled' : '' }} class="btn btn-default btn-xs">
                            ...
                            @foreach ($rootNode->getDescendants() as $k => $node)
                                <div class="list-group-item">
                                    ...
                                    <div class="pull-right section-editbox {{ Input::get('edit') === 'true' ? 'disnone' : 'disnone' }}">
                                        ...
                                        {!! Form::open(['url' => url('/admin/topic/node-destroy'), 'id' => ('form-node-' . $node->id), 'class' => 'hidden', 'method' => 'post']) !!}
                                            <input type="hidden" name="id" value="{{ $node->id }}">
                                            {!! Form::submit('Submit', ['class' => 'btn btn-xs']) !!}
                                        {!! Form::close() !!}
                                        &nbsp;&nbsp;

                                        <a href="{{ url('admin/topic/node-move-left?id=' . $node->id) }}" {{ $k === 0 ? 'disabled' : '' }} class="btn btn-default btn-xs">
                                            <i class="glyphicon glyphicon-arrow-up"></i>
                                        </a>
                                        <a href="{{ url('admin/topic/node-move-right?id=' . $node->id) }}" {{ $k === ($rootNode->getDescendants()->count() - 1) ? 'disabled' : '' }} class="btn btn-default btn-xs">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <br>
                {!! Form::open(['url' => url('/admin/topic/node-add'), 'id' => 'form-node-add', 'class' => 'form form-horizontal', 'method' => 'post']) !!}
...
<div class="modal fade" id="nodeRenameModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <br>
                {!! Form::open(['url' => url('/admin/topic/node-rename'), 'id' => 'form-node-rename', 'class' => 'form form-horizontal', 'method' => 'post']) !!}
